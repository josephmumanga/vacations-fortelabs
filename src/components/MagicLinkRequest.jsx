...
export default function MagicLinkRequest({ onSuccess, onBack }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data, error } = await api.requestMagicLink(email.trim().toLowerCase());
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm mt-1">
                Check your email ({email}) for a magic link to sign in. The link will expire in 15 minutes.
              </p>
            </div>
          </div>
        </div>
